...
- Discovery: `ModernComboBoxStyle` was referenced by `src/POS.Wpf/Views/SupplierManagementWindow.xaml:16` but defined only at `src/POS.Wpf/Views/EmployeeManagementWindow.xaml:106` inside that Window's resources. Runtime smoke then identified `DetailLabelStyle` and `DetailValueStyle` as the next same-scope defects used by Supplier windows. `SupplierEditorWindow` had no `ModernComboBoxStyle` reference.
- Fix: moved the three existing shared styles to `src/POS.Wpf/Themes/Controls.xaml`, which `src/POS.Wpf/App.xaml` merges after Colors/Typography, removed Employee-local duplicates, and added shared Supplier badge/DataGrid/danger-action styles. No resource was changed to DynamicResource and no style was duplicated in Supplier windows.
- Approved UI implementation is covered by source contract assertions for master-detail proportions/structure, compact Add icon/text grouping, no-selection/inactive semantic states, status text, AutomationProperties and shared resource scope. Runtime regression `SupplierUiContractTests.Supplier_windows_load_with_production_resources_and_dialog_path_on_sta` executes on STA with `App.InitializeComponent()` and production DI, constructs both Supplier windows and opens/loads/closes the editor. Supplier focused Release is `22/22 PASS`; combined focused Supplier/RBAC/UI is `40/40 PASS`.
- Final hotfix regression uses real STA layout on `1180x760`, `1080x640` and `1920x1000`: refresh button `ActualWidth >= MinWidth`, content `DesiredSize` stays inside the button, and both active/inactive status badges are center-aligned, `26–28.5 DIP` high and within the status column. Supplier focused Release is `22/22 PASS` after the hotfix.
- Responsive regression additionally reads all five Supplier column `ActualWidth` values at those sizes, asserts no grid overflow, keeps Phone/Status within bounded widths, verifies Name/Contact remain star-configured with Contact capped, and verifies the Status header and badge share centered alignment.
- Explicit Release build/apphost: `dotnet build POS.Enterprise.slnx --configuration Release --no-restore -m:1 -nr:false -p:BuildInParallel=false` plus WPF `UseAppHost=true` — PASS, `0 warning / 0 error`.
- Normal-Windows Official `scripts/Test-QualityGate.ps1` without `-SkipEfCheck`: Release restore/build, `1583 PASS / 0 FAIL / 0 SKIP`, vulnerability scan 5/5, EF pending-model and Git checks PASS; output included `QUALITY GATE PASSED`. Separate sandbox baseline remains `1577 PASS / 6 FAIL / 0 SKIP / 1583`, with only the six known DPAPI/VietQR/Remembered Login environment failures; no secure-storage test was filtered or weakened.
- Binary: `D:\Projects_1\POS_Enterprise_DotNet\src\POS.Wpf\bin\Release\net10.0-windows\POS.Enterprise.exe`; `POS.Enterprise.dll` SHA-256 `102B0C183457B892E102BE1D7CBC685EC5C45A44D59CF408AE888AABD1AC923C`, MVID `85a1635c-622f-4161-b888-238033d64428`, LastWriteTime `2026-09-04T20:59:24.6924810+07:00`. No real database was accessed or updated. Product Owner manual retest passed the documented normal/maximized Supplier UI checks; DPI 125/150, physical UI, InventoryStaff/Cashier click-through and physical audit/hardware remain deferred.
