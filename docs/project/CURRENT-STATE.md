...
- Audit thêm action 17–20 (`SupplierCreated`, `SupplierUpdated`, `SupplierDeactivated`, `SupplierReactivated`), snapshot chỉ gồm code + name, không ghi contact details; mutation và success audit cùng transaction/save boundary, no-op không audit. Constraint migration mở từ `1..16` lên `1..20` và bảo toàn LoginFailed action 16.
- Manual R6.1 evidence ban đầu trên normal Windows profile là `FAIL/BLOCKED` ngay bước 1: Administrator bấm “Nhà cung cấp” nhưng Supplier window không mở; Shell vẫn ở “Sản phẩm và tồn kho” và báo thiếu `ModernComboBoxStyle`.
- Discovery xác nhận `ModernComboBoxStyle` được tham chiếu tại `src/POS.Wpf/Views/SupplierManagementWindow.xaml:16` nhưng trước đó chỉ được định nghĩa trong `EmployeeManagementWindow.Resources`; `DetailLabelStyle` và `DetailValueStyle` cũng có cùng lỗi scope. `SupplierEditorWindow` không tham chiếu `ModernComboBoxStyle`, nhưng dùng hai detail styles đó.
- UI mockup master-detail đã triển khai: header hai phía, search/filter card, danh sách trái khoảng 64%, detail phải tối thiểu 420 px, DataGrid sáng có selection/focus semantic, badge trạng thái, ba detail cards, inactive banner, no-selection state và action footer cố định. Initials/total/metadata lấy từ ViewModel; shared badge/DataGrid/danger styles nằm trong `Controls.xaml` và App merge hiện hữu.
- Final UI hotfix đã xử lý hai lỗi hình ảnh: badge status từng stretch theo chiều cao cell và chật ngang do cell padding/cột; nút refresh từng bị giới hạn bởi toolbar column `Width=116` cộng margin/padding. Badge hiện center, `MinHeight=26`, `Padding=10,3`, status cell padding `4,5`, cột status `MinWidth=140`; refresh dùng column `Auto`, `MinWidth=112`, balanced padding và cùng MDL2 icon/text container.
- Responsive alignment hotfix giữ nguyên thứ tự cột nhưng giới hạn các cột phụ: Mã `110..140`, Tên `*` `MinWidth=90`, Liên hệ `*` `90..200`, Điện thoại `125..155`, Trạng thái `150..170`; Supplier header Trạng thái dùng style scoped căn giữa, DataGrid Supplier dùng stretch content và không overflow ở các layout regression `1180x760`, `1080x640`, `1920x1000`.
- Product Owner đã manual retest binary Release: management/editor mở được, normal/windowed PASS, maximized PASS, năm cột cân đối, header Trạng thái cùng trục với badge, badge compact và nút Làm mới không clip/đè. Tổng thể Supplier Master được chấp nhận.
- Runtime STA smoke dựng được `SupplierManagementWindow`, resolve shared resources và mở/đóng `SupplierEditorWindow` qua production dialog service. Supplier focused Release: `22/22 PASS`; combined Supplier/RBAC/UI filter: `40/40 PASS`.
- Normal-Windows Official Quality Gate: `1583 PASS / 0 FAIL / 0 SKIP`, Release build, vulnerability scan, EF pending-model và Git checks PASS; đã xuất hiện `QUALITY GATE PASSED`. Sandbox historical run `1577 PASS / 6 FAIL / 0 SKIP / 1583` vẫn được tách riêng là sáu lỗi DPAPI/VietQR/Remembered Login môi trường, không sửa/lọc.
- R6.1 manual UI acceptance đã PASS theo bằng chứng PO nêu trên. Không ghi manual PASS cho DPI 125/150, physical UI, InventoryStaff/Cashier click-through hoặc physical audit/hardware vì chưa có bằng chứng trực tiếp. R6.2 Purchase Receipt, R6.3 Batch/Lot và các phần R6 sau vẫn NOT STARTED/HOLD.
...
## R6.2A Purchase Order foundation — CLOSED / COMMITTED / PUSHED — 2026-09-04

- Purchase Order is implemented as a first-class aggregate with Draft, Ordered and Cancelled lifecycle states, immutable ordered identity snapshots, bounded VND integer money, checked totals, optimistic concurrency and no Delete API.
- Application, persistence, cryptographic UTC PO numbering, RBAC, sanitized audit actions and forward-only migration `20260904150256_AddPurchaseOrdersFoundation` are implemented. Production composition resolves `IPurchaseOrderService` to `AuthorizedPurchaseOrderService`; no Purchase Order WPF navigation or UI was added.
- R6.2A contains no Goods Receipt, InventoryMovement, stock/cost update, Product.SupplierId or real-database operation. The migration has not been applied to a real database.
- Product Owner normal-Windows release evidence is `1608/1608 PASS`, `0 failed`, `0 skipped`; EF pending-model passed; `git diff --check` passed; and the Quality Gate script printed `QUALITY GATE PASSED`. Release build succeeded with three environmental `NU1900` warnings because the NuGet vulnerability service index could not be loaded; this is not a zero-warning build claim.
- The vulnerability command enumerated all five projects and found no vulnerable package in the available result, but advisory retrieval failed because of NuGet/network access. This scan is `INCONCLUSIVE`, not PASS. No R6.2A package/configuration/lock-file change exists; the last known successful vulnerability scan is the R6.1 baseline.
- The Quality Gate script can currently print PASS despite `NU1900`; this is recorded as a known release-gate hardening item and `scripts/Test-QualityGate.ps1` was not changed in R6.2A. Release apphost publish succeeded. R6.2B and R6.2C have not started.
