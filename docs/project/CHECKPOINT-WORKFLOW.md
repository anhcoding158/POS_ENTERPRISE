...
- Current checkpoint: R2.1B — Single-instance Application Implementation & Automated Tests. R2.1A discovery/baseline is Completed; R2.1 implementation, automated tests and manual multi-process/crash acceptance are Not Started. R1.3 and the entire R1 are Closed / Committed / Pushed / Git-clean at `b9e382550e2e4abcf7a93ed6c5352322dc967668`; live Jenkins build #5 verified the owner-approved native `POS.Enterprise.*` contract on exact revision `8bebb3ebc2b61c4de2fd8d97dc4c0b6944281bb6`.
...
- R1.3 implementation/local verification and live Jenkins build #5 are PASS with native `POS.Enterprise.*` artifacts; the ZIP smoke test used an existing Windows profile and clean-profile first-run remains Not Revalidated. R1.3 and the entire R1 are Closed / Committed / Pushed / Git-clean at `b9e382550e2e4abcf7a93ed6c5352322dc967668`. R2 is In Progress at current executable checkpoint R2.1B; R2.1A discovery/baseline is Completed and R2.1 implementation, automated tests and manual multi-process/crash acceptance are Not Started. R2.2–R2.4 remain Not Started.
...
The historical Context Pack is export-only under ignored `artifacts/project-context/`. R2.1A discovery/baseline completed without implementation changes, build/test/gate reruns or Jenkins activity in this reconciliation turn. The next checkpoint is R2.1B — Single-instance Application Implementation & Automated Tests. No mutex/IPC implementation, automated single-instance tests, crash recovery acceptance or foreground activation acceptance is claimed as completed.
