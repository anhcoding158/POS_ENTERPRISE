...
- **Current/next checkpoint:** R2.4 — Disk Space and Database Growth — IN PROGRESS. R2.4A Typed Storage Snapshot Foundation is COMPLETED; R2.4B is next and NOT STARTED. R2.4A adds metadata-only typed database/sidecar/volume snapshots and overflow-safe preflight policy only; startup integration, UI, backup/restore and cleanup are not implemented.
