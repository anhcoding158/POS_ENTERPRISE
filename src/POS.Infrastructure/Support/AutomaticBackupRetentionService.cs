...
public sealed class AutomaticBackupRetentionService(AutomaticBackupPathProvider paths, AutomaticBackupPolicy policy, IStoreSettingsStore? settingsStore = null)
{
    public Task<AutomaticBackupRetentionResult> PruneAsync(string protectedArtifactIdentifier, CancellationToken cancellationToken = default)
    {
        ...
        var effectiveRetention = settingsStore?.Current.Retention;
        var latestCount = effectiveRetention?.LatestCount ?? policy.RecentRetentionCount;
        var weeklyCount = effectiveRetention?.WeeklyCount ?? policy.WeeklyRetentionCount;
        var monthlyCount = effectiveRetention?.MonthlyCount ?? policy.MonthlyRetentionCount;
        var maximumTotalBytes = effectiveRetention?.MaximumTotalBytes ?? policy.MaximumTotalBytes;
        try
        {
            ...
            foreach (var file in files.Take(Math.Max(0, latestCount))) keep.Add(file.Name);
            foreach (var group in files.Where(x => !keep.Contains(x.Name)).GroupBy(x => Week(Parse(x.Name)!.Value))
                .OrderByDescending(x => x.Key.Year).ThenByDescending(x => x.Key.Week).Take(Math.Max(0, weeklyCount)))
            ...
            foreach (var group in files.Where(x => !keep.Contains(x.Name)).GroupBy(x => (Parse(x.Name)!.Value.Year, Parse(x.Name)!.Value.Month))
                .OrderByDescending(x => x.Key.Year).ThenByDescending(x => x.Key.Month).Take(Math.Max(0, monthlyCount)))
                keep.Add(group.OrderByDescending(x => Parse(x.Name)).ThenByDescending(x => x.Name, StringComparer.Ordinal).First().Name);
            ...
            if (count > keep.Count || total > (ulong)maximumTotalBytes) warnings.Add("Retention chưa thể đạt quota; các snapshot GFS bắt buộc vẫn được giữ.");
        }
